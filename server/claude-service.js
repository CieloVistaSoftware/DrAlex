...
// Enhanced CORS configuration to fix connection issues
const corsOptions = {
  origin: function (origin, callback) {
    // Allow requests with no origin (like mobile apps, Postman, or file://)
    if (!origin) return callback(null, true);
    
    // Allow all localhost ports, 127.0.0.1, and file protocol
    ...
    callback(null, true); // Allow all for development - change for production
  },
...
app.get('/api/chat', (req, res) => {
  res.status(405).json({
    error: 'GET method not supported for /api/chat. Use POST with a message payload.'
  });
});
...
app.get('/api/test-image', (req, res) => {
  res.status(405).json({
    error: 'GET method not supported for /api/test-image. Use POST with an image file.'
  });
});
...
app.listen(PORT, () => {
  ...
  console.log(`Image upload test: http://localhost:${PORT}/api/test-image`);
  ...
  console.log('✅ Enhanced CORS enabled for all origins (development mode)');
});
