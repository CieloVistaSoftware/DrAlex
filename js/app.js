// Enhanced Application Class with JSON-driven content and Welcome Header
if (!window.EyeCareApp) {
  class EyeCareApp {
    constructor() {
      this.config = null;
    ...
    generateHomePage() {
      ...
      // Generate hero section with optional image
      const heroImageHTML = hero.image ? `
        <div class="hero-image ${hero.image.position === 'left' ? 'hero-image-left' : 'hero-image-right'}">
          <img src="${hero.image.src}" alt="${hero.image.alt}" class="hero-img" loading="lazy">
        </div>
      ` : '';
      
      const heroContentClass = hero.image ? 'hero-content-with-image' : '';
      
      return `
        <!-- Welcome Header aligned with About page header -->
        <div class="page-welcome-header">
          <h1 class="welcome-title">Welcome to Site Eternal Hospitals</h1>
          <div class="copyright-notice">© ${new Date().getFullYear()} SiteEternalHospitals. All rights reserved.</div>
        </div>

        <section class="content-section">
          <div class="hero-section ${hero.image ? 'hero-with-image' : ''}">
            <div class="hero-content ${heroContentClass}">
    ...
    generateChatPage() {
      ...
      return `
        <section class="content-section">
          ...
          <!-- Fallback if chat component fails -->
          <div class="chat-fallback" style="display: none;">
            <div class="content-card">
              <h3 class="card-title">Chat Currently Unavailable</h3>
              <p class="card-description">
                Our chat service is temporarily unavailable. Please contact us directly:
              </p>
              <div class="chat-actions" style="margin-top: 1rem;">
                ${this.config.contact?.phones?.[0] ? `
                  <button class="cta-button" onclick="window.location.href='tel:${this.config.contact.phones[0].number}'">
                    📞 Call ${this.config.contact.phones[0].number}
                  </button>
                ` : ''}
                <button class="cta-button secondary" onclick="window.location.href='#contact'">
                  📧 Contact Form
                </button>
              </div>
            </div>
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
      if (!window.eyeCareAppInstance) {
        window.eyeCareAppInstance = new EyeCareApp();
      }
    });
  } else {
    if (!window.eyeCareAppInstance) {
