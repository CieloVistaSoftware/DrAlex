...
test.describe('Improved Chat Widget', () => {
  test('should handle long messages and UI updates', async ({ page }: { page: Page }) => {
    ...
    if (!userMsg) throw new Error('User message not found in shadow DOM');
    const userMsgText = await userMsg.textContent();
    expect(userMsgText).toContain(longMessage);
    ...
    if (!botMsg) throw new Error('Bot message not found in shadow DOM');
